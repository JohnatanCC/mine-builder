...
const needsTint = (t: BlockType) =>
  t === "grass" || t.endsWith("_leaves");

const tintFor = (t: BlockType) =>
  t === "grass" ? TINT_GRASS : TINT_LEAVES;
...
export function BlockIcon({ type, size = 42, className, title }: Props) {
  ...
  React.useEffect(() => {
    let alive = true;
    if (!needsTint(type)) { setSrc(baseURL); return; }
    tintImageURLToDataURL(baseURL, tintFor(type))
      .then((u) => { if (alive) setSrc(u); })
      .catch(() => { if (alive) setSrc(baseURL); });
    return () => { alive = false; };
  }, [type, baseURL]);
  ...
  return <img src={src} alt={type} title={title ?? type} style={style} className={className} />;
}
